...
const MODEL_PATTERNS = [
  /^claude-\d-\d-sonnet-\d{8}$/,
  /^claude-\d-opus-\d{8}$/,
  /^claude-\d-\d-opus-\d{8}$/,
  /^claude-\d-haiku-\d{8}$/,
  /^claude-\d-\d-haiku-\d{8}$/,
  /^sonnet$/,
  /^opus$/,
  /^haiku$/,
  /^inherit$/,
...
async function validateMarketplace(validator) {
  ...
  if (fs.existsSync(schemaPath)) {
    try {
      ...
      validator.warn(
        `Could not validate schema: ${error.message}`,
        "marketplace.json",
      );
    }
  }
...
function validateAgentFile(filePath, validator) {
  ...
  try {
    ...
    if (frontmatter.model) {
      const isValid = MODEL_PATTERNS.some((pattern) =>
        pattern.test(frontmatter.model),
      );
      if (!isValid) {
        validator.error(
    ...
    if (!frontmatter.__content || frontmatter.__content.trim().length === 0) {
      validator.warn(
        "Agent file has no content beyond frontmatter",
        relativePath,
      );
    }
  } catch (error) {
...
function validateCommandFile(filePath, validator) {
  ...
  try {
    ...
    if (!frontmatter.__content || frontmatter.__content.trim().length === 0) {
      validator.warn(
        "Command file has no content beyond frontmatter",
        relativePath,
      );
    }
  } catch (error) {
